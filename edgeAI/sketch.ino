...
/* Includes ---------------------------------------------------------------- */
#include <skateboard_motion_inferencing.h>
...
void turn_on_leds(int prediction);
void turn_off_leds();
...
void setup()
{
    ...
    while (!Serial);
    Serial.println("Edge Impulse Skateboard Motion Inference\r\n");
...
void loop()
{
    ...
    float predictionValue = result.classification[0].value;

    // print the predictions
    ei_printf("Predictions (DSP: %d ms., Classification: %d ms., Anomaly: %d ms.):\r\n",
        result.timing.dsp, result.timing.classification, result.timing.anomaly);
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
        if (result.classification[ix].value > predictionValue) {
            predictionIndex = ix;
            predictionValue = result.classification[ix].value;
        }
        ei_printf("%s: %.5f\r\n", result.classification[ix].label, result.classification[ix].value);
    }
    ei_printf("Got out of the loop %d \n", predictionIndex);
    turn_on_leds(predictionIndex);
